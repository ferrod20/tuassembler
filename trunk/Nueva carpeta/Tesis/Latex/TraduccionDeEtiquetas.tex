...
\STATE \textbf{VBN|VBD}:
\STATE 
\STATE Si TnT asignó \textbf{VBN o VBD}, asignar la etiqueta TnT
\STATE Si no asignar la etiqueta VB
\STATE 
...
Luego de ejecutado este proceso el análisis de comparación contra las etiquetas asignadas por TnT dió un 87.6\% de aciertos .
...
En la entrada presentada la palabra definida es \emph{celebrate} y las formas derivadas son \emph{celebrates, celebrating} y \emph{celebrated}. Con esta información y la etiqueta asignada por \emph{Cobuild} (\emph{verb + object, or verb + object + adjunct (as/for)}) se pueden inferir y generar etiquetas \emph{Penn Treebank} para dichas formas. 

En lugar de guardar la etiqueta \emph{Penn Treebank} VB correspondiente a \emph{verb} para la palabra \emph{celebrating}, guardaríamos la etiqueta VBG\footnote{verbo gerundio o presente participio} que contiene más información gramatical.
