...
\\
\\
\emph{\small Book/VB that/DT flight/NN ./.\\
...
La información presentada en este subcapítulo está basada en \cite[sub~chapter~5.5]{Jurafsky}
...
Un ejemplo de característica para el problema de etiquetado morfosintáctico sería la siguiente: 

\begin{equation*}
f(w, t) = \left\{
\begin{array}{rl}
1 &  \text{si } w = \text{the y } t = \text{DT}\\
0 &  \text{en otro caso} 
\end{array} \right.
\end{equation*}

que indica que f toma el valor 1 si la palabra $w$ es the y está etiquetada como un determinante (DT).

El trabajo de Máxima Entropía más relevante fue desarrollado por Ratnaparkhi \cite{Ratnaparkhi} y aplicado al etiquetado morfosintáctico. También se ha utilizado en la determinación del ligamiento preposicional \cite{Ratnaparkhi}, en el análisis sintáctico parcial \cite{Osborne} y en la desambiguación de sentidos \cite{Suarez}.