...
Podemos apreciar que Cobuild asigna la etiqueta \emph{qualitative adjective} a la palabra \emph{acid}. 
...
Para realizar el proceso de traducción de etiquetas se construyó una tabla de conversión Cobuild-Penn Treebank. Ante la falta de documentación presente en las etiquetas de Cobuild se realizó un análisis que arrojó como resultado la existencia de más de 4000 etiquetas diferentes. 

A partir de este hecho se identificaron las etiquetas de Cobuild más frecuentes y se seleccionó la etiqueta Penn Treebank equivalente para cada una de ellas, obteniendo una tabla de traducción ad hoc que se presenta a continuación
...
Los mayores focos de error que no puedieron ser aplacados corresponden a palabras etiquetadas como VBD cuando son VBN y viceversa. Estas etiquetas son difícilmente desambigüables automáticamente.
...
A continuación se muestran ejemplos del resultado de este proceso:
...
En el ejemplo presentado la palabra definida es \emph{celebrate} y las formas derivadas son \emph{celebrates, celebrating} y \emph{celebrated}. Con esta información y la etiqueta asignada por Cobuild en el ejemplo (\emph{verb + object, or verb + object + adjunct (as/for)}) se pueden inferir y generar etiquetas \emph{Penn Treebank} con información adicional para las formas derivadas. 

En el ejemplo ocurre la forma flexionada \emph{celebrating} con la etiqueta \emph{verb + object, or verb + object + adjunct (as/for)}. Reconociendo \emph{celebrating} podemos adicionar información extra a la etiqueta; en vez de guardar la etiqueta \emph{Penn Treebank} correspondiente a \emph{verb} (VB), guardaríamos la etiqueta  VBG\footnote{verbo gerundio o presente participio} que contiene más información gramatical que VB.
...
\noindent 2. 
Forma derivada \emph{faults} inferida como NNS a partir de la palabra \emph{fault} y la etiqueta NN:
